...
namespace CurveManager {
    ...
    CurveBuilder::CurveBuilder(const json& data, MarketStore& marketStore)
    : data_(data), marketStore_(marketStore) {
    ...
    boost::shared_ptr<YieldTermStructure> CurveBuilder::buildPiecewiseCurve(
        const std::string& curveName, const json& curveParams) {
        ...
        boost::shared_ptr<YieldTermStructure> curvePtr(
            new PiecewiseYieldCurve<Discount, LogLinear>(qlRefDate, helpers, dayCounter));
        return curvePtr;
    };

    boost::shared_ptr<YieldTermStructure> CurveBuilder::buildDiscountCurve(
        const std::string& curveName, const json& curveParams) {
    ...
    boost::shared_ptr<YieldTermStructure> CurveBuilder::buildFlatForwardCurve(
        const std::string& curveName, const json& curveParams) {
        DayCounter dayCounter   = parse<DayCounter>(curveParams.at("DAYCOUNTER"));
        ...
        Date qlRefDate          = Settings::instance().evaluationDate();
        boost::shared_ptr<YieldTermStructure> curvePtr(
            new FlatForward(qlRefDate, rate, dayCounter, compounding, frequency));
        return curvePtr;
    };
    ...
    void CurveBuilder::updateQuotes(const json& prices) {
        ...
        for (const auto& pair : prices)
            if (!marketStore_.hasQuote(pair.at("NAME")))
                throw std::runtime_error("No quote found for " + pair.at("NAME"));

        marketStore_.freeze();
        for (const auto& pair : prices) {
            Handle<Quote> handle = marketStore_.getQuote(pair.at("NAME"));
            boost::shared_ptr<SimpleQuote> quote =
                boost::static_pointer_cast<SimpleQuote>(handle.currentLink());
            quote->setValue(pair["VALUE"]);
        }
        marketStore_.unfreeze();
    }

    std::vector<boost::shared_ptr<RateHelper>> CurveBuilder::buildRateHelpers(
        ...
        for (auto const& helper : rateHelperVector) {
            ...
            if (type == "DEPOSIT") {
                boost::shared_ptr<DepositRateHelper> h =
                    JsonToObjectWrapper<DepositRateHelper>(helper, priceGetter);
                ...
                auto h = JsonToObjectWrapper<SwapRateHelper>(helper, priceGetter, indexGetter,
                                                             curveGetter);
                h->unregisterWith(Settings::instance().evaluationDate());
                rateHelpers.push_back(h);
            } else if (type == "OIS") {
                auto h = JsonToObjectWrapper<OISRateHelper>(helper, priceGetter, indexGetter,
                                                            curveGetter);
                h->unregisterWith(Settings::instance().evaluationDate());
                rateHelpers.push_back(h);
            } else if (type == "XCCY") {
                auto h = JsonToObjectWrapper<CrossCcyFixFloatSwapHelper>(helper, priceGetter,
                                                                         indexGetter, curveGetter);
                h->unregisterWith(Settings::instance().evaluationDate());
                rateHelpers.push_back(h);
            } else if (type == "XCCYBASIS") {
                auto h = JsonToObjectWrapper<CrossCcyBasisSwapHelper>(helper, priceGetter,
                                                                      indexGetter, curveGetter);
                h->unregisterWith(Settings::instance().evaluationDate());
                rateHelpers.push_back(h);
            } else if (type == "TENORBASIS") {
                auto h = JsonToObjectWrapper<TenorBasisSwapHelper>(helper, priceGetter, indexGetter,
                                                                   curveGetter);
                h->unregisterWith(Settings::instance().evaluationDate());
                rateHelpers.push_back(h);
            } else {
    ...
    boost::shared_ptr<IborIndex> CurveBuilder::buildIndex(const std::string& name) {
        if (!marketStore_.hasIndex(name)) {
            ...
            if (type == "OVERNIGHT") {
                boost::shared_ptr<IborIndex> index(new OvernightIndex(
                    name, fixingDays, currency, calendar, dayCounter, curveHandle));
                marketStore_.addCurveHandle(name, curveHandle);
                marketStore_.addIndex(name, index);
            } else if (type == "IBOR") {
                boost::shared_ptr<IborIndex> index(new IborIndex(name, tenor, fixingDays, currency,
                                                                 calendar, convention, endOfMonth,
                                                                 dayCounter, curveHandle));
                marketStore_.addCurveHandle(name, curveHandle);
                marketStore_.addIndex(name, index);
            } else {
